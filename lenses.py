...
def burry_lens(fin: Dict[str, float], mult: Dict[str, float], roic: Dict[str, float]) -> Dict:
    """Deep value: cheap on EV/EBITDA, decent FCF yield."""
    checks = {
    ...
    return {
        "verdict": "PASS" if all(passed) else "MIXED" if any(passed) else "FAIL",
        "score": _score(passed),
        "checks": checks,
...
def greenblatt_lens(fin: Dict[str, float], mult: Dict[str, float], roic: Dict[str, float]) -> Dict:
    """Magic Formula: high ROC and cheap."""
    checks = {
        "ROC (Greenblatt) > 15%": _ok(roic.get("ROC (Greenblatt, EBIT / (NWC + Net PPE))"), 0.15, "gt"),
...
def buffett_lens(fin: Dict[str, float], mult: Dict[str, float], roic: Dict[str, float]) -> Dict:
    """Quality + compounding: ROIC and FCF coverage."""
    checks = {
        "ROIC > 12%": _ok(roic.get("ROIC (NOPAT / Invested Capital)"), 0.12, "gt"),
...
def einhorn_lens(fin: Dict[str, float], mult: Dict[str, float], roic: Dict[str, float]) -> Dict:
    """Relative value: EBIT/EV vs alternatives."""
    checks = {
